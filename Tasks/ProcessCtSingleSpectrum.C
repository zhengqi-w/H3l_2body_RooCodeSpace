...
// Run: root -l -b -q 'ProcessCtSingleSpectrum.C()'
...
static string GetJStr(const GeneralHelper::Json &j, const std::string &key, const std::string &def = "") {
  return j.contains(key) ? j[key].get<std::string>() : def;
}

int ProcessCtSingleSpectrum(const char *generalConfigPath = ""){
  string snapshotDir = "/Users/zhengqingwang/alice/run3task/H3l_2body_spectrum/ROOTWorkFlow/SnapShotsData/LHC23_PbPb_pass5_CustomV0s_HadronPID";
  string wpPath = "/Users/zhengqingwang/alice/run3task/H3l_2body_spectrum/ROOTWorkFlow/Outputs/MLProcess/LHC23_PbPb_pass5_CustomV0s_HadronPID/WorkingPoint/WorkingPoint_CtSingle.txt";
  string mcAo2d = "/Users/zhengqingwang/alice/data/derived/Hypertriton_2body/LHC23_PbPb_fullTPC/mc/apass5/LHC25g11_G4list/AO2D_CustomV0s.root";
  string reweightPtFile = "/Users/zhengqingwang/alice/run3task/H3l_2body_spectrum/ROOTWorkFlow/CodeSpace/Ploting_scrips/H3L_BWFit_Run3_23.root";
  string absorptionCorrFile = "/Users/zhengqingwang/alice/run3task/H3l_2body_spectrum/AbsorptionTrees/absorption_tree_x1.5.root";
  string dataTree = "O2hypcands";
  string mcTree   = "O2mchypcands";
  string absorTreename = "he3candidates";
  string isMatter = "both"; // "matter", "antimatter", "both"
  string basicSelectionDataForMCEff = "fTPCsignalPi<1000 && fCosPA>0.99 && fAvgClusterSizeHe > 5 && fDecRad > 0.5";
  string outFileDir = "/Users/zhengqingwang/alice/run3task/H3l_2body_spectrum/ROOTWorkFlow/Outputs/LHC23_PbPb_pass5_CustomV0s_HadronPID/Checks/LifeTime_0_80";

  if (generalConfigPath && std::string(generalConfigPath).size() > 0) {
    auto j = GeneralHelper::LoadJsonFile(generalConfigPath);
    if (j.contains("common") && j["common"].is_object()) {
      const auto &jc = j["common"];
      const auto &path = jc.contains("path") ? jc["path"] : GeneralHelper::Json::object();
      const auto &tree = jc.contains("tree_names") ? jc["tree_names"] : GeneralHelper::Json::object();
      const auto &tags = jc.contains("tags") ? jc["tags"] : GeneralHelper::Json::object();
      const auto &others = jc.contains("others") ? jc["others"] : GeneralHelper::Json::object();
      const auto &jct = j.contains("ct_single") ? j["ct_single"] : GeneralHelper::Json::object();
      const auto &sw = jct.contains("switches") ? jct["switches"] : GeneralHelper::Json::object();

      snapshotDir = GetJStr(path, "input_snapshot_dir", snapshotDir);
      mcAo2d = GetJStr(path, "mc_file_for_acceptance", mcAo2d);
      absorptionCorrFile = GetJStr(path, "mc_file_for_absorption", absorptionCorrFile);
      const string wpDir = GetJStr(path, "working_point_dir", "");
      if (!wpDir.empty()) wpPath = wpDir + "/WorkingPoint_CtSingle.txt";
      const string baseOut = GetJStr(path, "output_dir", "");
      if (!baseOut.empty()) {
        string period = GetJStr(tags, "period", "Run3");
        string mark = GetJStr(tags, "period_mark", "default");
        outFileDir = baseOut + "/" + period + "_" + mark + "/Checks/CtSingleSpectrum";
      }
      dataTree = GetJStr(tree, "data", dataTree);
      mcTree = GetJStr(tree, "mc", mcTree);
      absorTreename = GetJStr(tree, "absorption", absorTreename);
      isMatter = GetJStr(sw, "is_matter", isMatter);
      basicSelectionDataForMCEff = GetJStr(others, "basic_selection_data_for_mc_eff", basicSelectionDataForMCEff);
      string extraDataSel = GetJStr(jct, "additional_data_selection", "");
      if (!extraDataSel.empty()) basicSelectionDataForMCEff += " && (" + extraDataSel + ")";
      reweightPtFile = GetJStr(path, "reweight_pt_file", reweightPtFile);
    }
  }
  ...
  if(dataFiles.empty()){ ::Error("CtSingleSpectrum","No data_ct_* files found under %s", snapshotDir.c_str()); return 1; }
  // unique sort edges
  ...
  if(wpMap.empty()){ ::Error("CtSingleSpectrum","WP map empty"); return 2; }

  // acceptance from AO2D MC
  if(gSystem->AccessPathName(mcAo2d.c_str())){ ::Error("CtSingleSpectrum","MC AO2D missing: %s", mcAo2d.c_str()); return 3; }
  ROOT::EnableImplicitMT();
  TFile mcFile(mcAo2d.c_str(), "READ");
  if(mcFile.IsZombie()){ ::Error("CtSingleSpectrum","Cannot open MC AO2D: %s", mcAo2d.c_str()); return 4; }
  TChain mcChain(mcTree.c_str());
  GeneralHelper::fillChainFromAO2D(mcChain, &mcFile);
  if(mcChain.GetEntries()<=0){ ::Error("CtSingleSpectrum","MC chain empty (tree %s)", mcTree.c_str()); return 5; }
  ...
  if(!accHist){ ::Error("CtSingleSpectrum","Acceptance histogram null"); return 6; }
