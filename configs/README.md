# `general_config.json` Configuration Guide

This document describes the parameters in `general_config.json`. The configuration is organized by workflow scope and stage: `common`, `execution`, `preprocess`, `analysis`, `checks`, and `output`. A typical workflow command is:
...
| `balstwave_parameters_initial` | Legacy Blast-Wave initial-parameter block kept for compatibility. The integrated-yield fit now primarily uses `analysis.fit.integral_fit_parameters`. |
...
| `use_score_efficiency_fallback` | Whether to generate fallback score-efficiency arrays when missing. The default is usually `false` so training problems remain visible. |
| `period_weight_mode` | Period weighting mode in combined-period training. `equal_period` gives equal weight to each period; empty/disabled means no period reweighting. |
...
| `centrality_ranges` | List of centrality ranges to which the override applies, for example `[[0,5],[5,10]]`. |
...
This block is used only when `use_training_overrides=true`.
...
| `use_spectrum` | If `true`, expected signal is computed from the spectrum function. If `false`, expected signal is estimated from a constant fit to `3S/BDTefficiency`. |
...
| `target_pt_range`, `target_ct_range` | Optional pT/ct restrictions for WP processing. Empty arrays mean no restriction. |
...
| `integral_fit_func` | Nominal integrated-yield fit function. The standard choice is currently `fBGBW`. |
| `integral_fit_parameters` | Initial values and limits for the nominal integrated-yield function. |
...
| `integral_fit_funcs` | Function list used for integrated-yield fit-function systematics. |
